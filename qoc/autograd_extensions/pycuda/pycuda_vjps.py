...
from qoc.standard.extensions.autograd_extensions.pycuda.pycuda_wrapper import (
    ...
    stack_gpu, subtract_gpu, sum_gpu, trace_gpu,
)
...
defvjp(power_gpu,
       lambda ans, x, y: unbroadcast_f(x, lambda g: g * y * x ** where_gpu(y, y - 1, 1.)),
       lambda ans, x, y: unbroadcast_f(y, lambda g: g * log_gpu(replace_zero(x, 1.)) * x ** y))
defvjp(subtract_gpu,
       lambda ans, x, y : unbroadcast_f(x, lambda g: g),
       lambda ans, x, y : unbroadcast_f(y, lambda g: -g))
defvjp(sum_gpu,
...
def replace_zero(x, val):
    """
    This function is analogous to its counterpart in autograd.
    """
...
def unbroadcast_f(target, f,):
    """
    This function is analgogous to its counterpart in autograd.
    """
