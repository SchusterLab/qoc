...
def simulation(fock, dim_c , dim_trans, w_c, w_t, anharmonicity, g, evolution_time, step, mode):
    ...
    projector=np.zeros((dim_c*dim_trans,dim_c*dim_trans))
    projector[0][0]=1
    COSTS = [ForbidStatesprojector(projector=np.array([projector]),system_eval_count=SYSTEM_EVAL_COUNT)]
...
pre=2*np.pi
dim=3
result = simulation(1, dim, 6, 6 * pre, 3 * pre, -0.225 * pre, 0.1 * pre, 100, 400,"AD")